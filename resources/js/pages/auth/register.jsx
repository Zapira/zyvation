...
export default function Register() {
    const { register, handleSubmit, errors, loading, onSubmit, onError } = RegisterHook();
    return (
    ...
    bg-white opacity-10 rounded-full "></div>
                        ...
                        <div className="mt-10 relative z-10">
                            <div className="flex items-center rounded-2xl bg-white/20 p-2 text-sm w-2/3 backdrop-blur-md">
                                ...
                                <span className="text-white">Gratis untuk memulai</span>
                            </div>
                            <h1 className="text-white font-bold text-3xl leading-snug mt-4">
                                Buat undangan impianmu sekarang!
                            </h1>

                            <p className="text-white/70 mt-4 max-w-md text-sm">
                                Bergabung dengan puluhan ribu pasangan yang telah mempercayakan momen spesial mereka kepada Zyvation.
                            </p>
                        </div>

                        <div className="space-y-3 pt-10 relative z-10 text-sm">

                            <div className="flex items-center text-white font-semibold">
                                <div className="w-8 h-8 bg-white/15 rounded-lg flex items-center justify-center"><GrTemplate /></div>
                                <span className="ml-3">Berbagai macam template gratis</span>
                            </div>

                            <div className="flex items-center text-white font-semibold">
                                <div className="w-8 h-8 bg-white/15 rounded-lg flex items-center justify-center">📊</div>
                                <span className="ml-3">Analisis dan laporan yang lengkap</span>
                            </div>

                            <div className="flex items-center text-white font-semibold">
                                <div className="w-8 h-8 bg-white/15 rounded-lg flex items-center justify-center">⚙️</div>
                                <span className="ml-3">Kemudahan dalam penggunaan</span>
                            </div>

                        </div>
                    </div>

                    <div className="w-full md:w-96 bg-white flex flex-col justify-center items-center mb-10">
                        <h3 className="text-2xl font-bold text-[#0b1f3a] mt-10">
                            Buat Akun Baru
                        </h3>

                        <p className="text-gray-500 text-sm mb-6">
                            Sudah punya akun? <Link to="/login" className="text-[#4F46E5] hover:underline">Masuk di sini</Link>
                        </p>

                        <form className="p-8 py-3" onSubmit={handleSubmit(onSubmit, onError)}>
                            <div className="flex gap-4 md:flex-row flex-col w-full">
                                <div className="mt-4">
                                    <label className="text-xs font-bold text-gray-700">Nama Depan</label>
                                    <input type="text"
                                        {...register('first_name', { required: 'Nama depan wajib diisi' })}
                                        className={`w-full mt-1 px-4 py-2 text-sm rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.first_name ? 'border-red-500' : ''}`}
                                        placeholder="Nama Depan" />
                                </div>
                                <div className="mt-4">
                                    <label className="text-xs font-bold text-gray-700">Nama Belakang</label>
                                    <input type="text"
                                        {...register('last_name')}
                                        className={`w-full mt-1 px-4 py-2 text-sm rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.last_name ? 'border-red-500' : ''}`}
                            ...
                            <div className="mt-4">
                                <label className="text-xs font-bold text-gray-700">Konfirmasi Kata Sandi</label>
                                <input type="password"
                                    {...register('password_confirmation', { required: 'Konfirmasi kata sandi wajib diisi' })}    
                                    className={`w-full mt-1 px-4 py-2 text-sm rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors.password_confirmation ? 'border-red-500' : ''}`}
                                    placeholder="••••••••" />
                            </div>
